...
static void launch_mul_mat_q(ggml_backend_ac4k_context & ctx, const mmq_args & args, cudaStream_t stream) {
...
template <ggml_type type>
void mul_mat_q_case(ggml_backend_ac4k_context & ctx, const mmq_args & args, cudaStream_t stream) {
    const int    id     = ggml_cuda_get_device();
    const int    cc     = ggml_cuda_info().devices[id].cc;
...
#define DECL_MMQ_CASE(type)                                                        \
    template void mul_mat_q_case<type>(ggml_backend_ac4k_context & ctx, const mmq_args & args, cudaStream_t stream) \
...
void ggml_cuda_mul_mat_q(
        ggml_backend_ac4k_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * ids, ggml_tensor * dst);
